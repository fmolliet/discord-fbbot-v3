...
export class Bot {
    ...
    private async handleInteration() : Promise<void> {
        ...
        this.client.on(Events.InteractionCreate, async (interaction)=> {
            ...
            LOG.info(`Recebendo interação: ${interaction.commandName}`)
    ...
    private async registerCommands(){
        ...
        const supportedCommands = this.commands.filter((command)=>command.hasSlashSupport)
		// The put method is used to fully refresh all commands in the guild with the current set
        LOG.info(`Started refreshing ${supportedCommands.size} application (/) commands.`);
