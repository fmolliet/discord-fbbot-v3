...
export default class MessageHandler {
    ...
    constructor(client: Client){
        this.client = client;
        globPromise('src/modules/**/*.ts').then((result: string[])=>{
            if (result.length < 1){
            ...
            for (const file of result) {
                import(file.replace('src/','../').replace('.ts','')).then((command: Command)=>{
                    this.commands.set(command.name, command);
                });
                
            }
            
            LOG.info(`${result.length} módulos de comandos carregados.`);
        });
    ...
    public async handle(message: Message){
        ...
        const args : Array<string> = message.content.slice(CONSTANTS.prefix.length).split(/ +/);
        ...
        if ( command.hasArgs && ( args.length === 0) ){
            message.reply(`está faltando informar algo parça! dá uma olhada usando o comando: \`${CONSTANTS.prefix}help ${command.name}\``);
            return;
        }
